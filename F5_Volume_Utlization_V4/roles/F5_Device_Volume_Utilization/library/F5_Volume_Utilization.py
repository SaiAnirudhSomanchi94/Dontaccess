            ...
            try:
                ...
                while i <= max_loops:
                    ...
                    if 'timed out' in output:
                        # logging.debug('SSH Connection timed out ')
                        time.sleep(.5)
                        val = 'break'
                    time.sleep(.5)
                    if '>' in output or '#' in output :
                    ...
                    if i == 61 or val == 'break':
                        # logging.debug('not successfull')
                        break
            except Exception as e:
                    response_content = str('Issue in Device Connectivity, Manual action is required-' )
                    # logging.debug(str('Issue in Device Connectivity, Manual action is required-' + str(i)))
                    decision = 0
        else:
...
def main():
    ...
    # logging.basicConfig(filename='logs/' + str(current_time) + '.log',
